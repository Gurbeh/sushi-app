...
const kSushiShareOrigin = 'https://sushi.app';

/// Pending [mediaSourceId] from a share deep link, keyed by catalog item id.
final sushiShareMediaSourceIdProvider = StateProvider.autoDispose.family<String?, String>((ref, catalogId) => null);
...
void sushiBufferShareMediaSource({
...
void sushiFlushBufferedShareMediaSource(WidgetRef ref) {
  ...
  if (mediaSourceId != null && mediaSourceId.isNotEmpty) {
    ref.read(sushiShareMediaSourceIdProvider(catalogId).notifier).state = mediaSourceId;
  }
}

/// Read buffered share [mediaSourceId] before flush (for pending route paths).
String? sushiPeekBufferedShareMediaSourceId(String catalogId) {
...
String sushiBuildShareUrl(String catalogId, {String? mediaSourceId}) {
  final id = catalogId.trim();
  final base = '$kSushiShareOrigin/share/$id';
  final msId = mediaSourceId?.trim();
...
String? sushiShareMediaSourceIdFromItem(ItemBaseModel item) {
...
bool sushiIsShareableItem(ItemBaseModel item) {
...
String? sushiMediaSourceIdFromShareUri(Uri uri) {
  final fromQuery = uri.queryParameters['mediaSourceId']?.trim();
  if (fromQuery != null && fromQuery.isNotEmpty) return fromQuery;
  return null;
}

/// Parse catalog id from `https://sushi.app/share/{id}` or `sushi:///share/{id}`.
String? sushiCatalogIdFromShareUri(Uri uri) {
...
bool sushiIsShareWebHost(Uri uri) {
  final host = uri.host.toLowerCase();
  return host == 'sushi.app' || host == 'www.sushi.app';
}

ItemBaseModel sushiApplyShareMediaSource(ItemBaseModel item, Ref ref) {
...
String? _resolveShareMediaSourceId(Ref ref, String catalogId) {
  ...
  return ref.read(sushiShareMediaSourceIdProvider(catalogId));
}
...
void _clearShareMediaSourceId(Ref ref, String catalogId) {
  ...
  ref.read(sushiShareMediaSourceIdProvider(catalogId).notifier).state = null;
}

MovieModel? sushiApplyShareMediaSourceToMovie(MovieModel? movie, Ref ref) {
  if (movie == null) return null;
  final applied = sushiApplyShareMediaSource(movie, ref);
  return applied is MovieModel ? applied : movie;
}

EpisodeModel? sushiApplyShareMediaSourceToEpisode(EpisodeModel? episode, Ref ref) {
  if (episode == null) return null;
  final applied = sushiApplyShareMediaSource(episode, ref);

/// Reports when cold-start splash exceeds [kSushiSlowSplashThresholdMs] (capture no-op).
const kSushiSlowSplashThresholdMs = 2000;

/// Tracks splash phases. Capture methods are no-ops (Sentry removed).
final class SushiSplashTiming {
  ...
  Future<void> finishAndReport({
    ...
    if (totalMs < kSushiSlowSplashThresholdMs) return;

    // Keep timing math so callers retain identical local behavior.

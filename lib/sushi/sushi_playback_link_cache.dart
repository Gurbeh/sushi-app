...
abstract final class SushiPlaybackLinkCache {
  ...
  static void putFromResponse(PlaybackInfoResponse? response) {
    ...
    for (final source in sources) {
      ...
      if (!sushiMediaSourceHasPlayableUrl(source)) continue;
...
bool _sourceIsCommittedPlayable(MediaSourceInfo source) {
  ...
  if (!sushiIsTelegramProviderLink(path)) return true;
...
bool _responseHasCommittedPlayableUrl(PlaybackInfoResponse response, {required String mediaSourceId}) {
  final source = sushiResolvePlaybackMediaSource(response, requestedMediaSourceId: mediaSourceId);
...
class _Entry {
  ...
  bool get isFresh => DateTime.now().difference(storedAt) < SushiPlaybackLinkCache.ttl;
}

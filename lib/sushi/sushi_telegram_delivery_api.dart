...
abstract final class SushiTelegramDeliveryApi {
  ...
  static Future<void> report({
    ...
    final token = SushiImageAuth.accessToken;
    ...
    try {
      ...
      if (response.statusCode >= 400) {
        debugPrint('SUSHI_TDLIB: delivery report rejected (${response.statusCode})');
      }
    } catch (e) {
      debugPrint('SUSHI_TDLIB: delivery report failed: $e');
    }
  }
  ...
  static Future<void> forget({required String locator}) async {
    ...
    try {
      ...
      debugPrint('SUSHI_TDLIB: delivery forget failed: $e');
    }
  }
  ...
  static Future<List<SushiTdlibProviderBot>> providerBots() async {
    final base = SushiEnv.apiBaseUrl;
    final token = SushiImageAuth.accessToken;
    if (base == null || token == null) {
      debugPrint('SUSHI_TDLIB: provider-bots skipped — no access token');
      return const [];
    }
    try {
      ...
      if (response.statusCode >= 400) {
        debugPrint('SUSHI_TDLIB: provider-bots rejected (${response.statusCode})');
        return const [];
      }
      ...
      final parsed = <SushiTdlibProviderBot>[];
      ...
      if (parsed.isEmpty && bots.isNotEmpty) {
        debugPrint('SUSHI_TDLIB: provider-bots parse skipped ${bots.length} row(s)');
      }
      return parsed;
    } catch (e) {
      debugPrint('SUSHI_TDLIB: provider-bots failed: $e');
      return const [];
    }
  }
  ...
  /// [_headers] plus [sushiReaderKindHeader] — report/forget must land under the kind THIS
  /// device actually read with, not whichever kind the backend defaults to for the account. See
  /// SushiReaderKindInterceptor's doc for why (multi-device accounts with both a session and a
  /// bot connected).
  static Future<Map<String, String>> _headersWithReaderKind(String token) async {
    final isBot = await SushiTdlibBridgeController.instance().isNativeSessionActuallyBot();
    return {
      ..._headers(token),
      sushiReaderKindHeader: isBot ? 'bot' : 'session',
    };
  }

  static SushiTdlibProviderBot? _parseProviderBot(Map entry) {
    final id = _providerBotId(entry['id']);
    final username = entry['username'];
    if (id == null || id <= 0 || username is! String || username.isEmpty) {
      return null;
    }
    return SushiTdlibProviderBot(id: id, username: username);
  }

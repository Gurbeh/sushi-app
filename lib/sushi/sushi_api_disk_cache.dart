...
abstract final class SushiApiDiskCache {
  ...
  static Future<SushiApiDiskCacheEntry?> read(String key) async {
    ...
    try {
      ...
      final entry = SushiApiDiskCacheEntry.fromJson(decoded);
  ...
  static Future<void> write(String key, SushiApiDiskCacheEntry entry) async {
    final dir = await _cacheDir();
    if (dir == null) return;
    final file = File('${dir.path}${Platform.pathSeparator}$key.json');
    try {
...
class SushiApiDiskCacheEntry {
  const SushiApiDiskCacheEntry({
  ...
  factory SushiApiDiskCacheEntry.fromJson(Map<String, dynamic> json) {

...
abstract final class SushiPlaybackPrefetch {
  ...
  static void scheduleForSeries(SushiRead read, SeriesModel? series, {bool once = false}) {
    ...
    final episode = sushiSeriesPlayableNextUp(series);
  ...
  static void scheduleForItem(
    ...
    // this whole prefetch pipeline is Jellyfin PlaybackInfo/sushi-be delivery-API specific and
  ...
  static Future<void> _run(
    ...
    if (sushiTdlibPlayInProgress()) {
      SushiStreamLog.event('playback_prefetch', fields: {
        'itemId': itemId,
    ...
    // Telegram 400 chat not found, and the client then spins 20s on sushi-tg://0/0.
    final botsReady = await SushiProviderBotsBootstrap.ensureReady();
    if (!botsReady) {
      SushiStreamLog.event('playback_prefetch', fields: {
        'itemId': itemId,
    ...
    try {
      ...
      final response = await sushiPollPlaybackInfoUntilReady(() {
  ...
  static Future<void> _warmTdlibFromResponse(SushiRead read, PlaybackInfoResponse body) async {
    final source = sushiResolvePlaybackMediaSource(body);
    final path = source?.path?.trim();
    if (path == null || !sushiIsTelegramProviderLink(path)) return;
    final parsed = sushiParseTelegramDeliveryPath(path);
    if (parsed == null) return;
    if (sushiTdlibPlayInProgress() || sushiTdlibPlayIsResolving(parsed.locator)) {
      SushiStreamLog.event('tdlib_warm', fields: {
        'url': SushiStreamLog.describeUrl(path),

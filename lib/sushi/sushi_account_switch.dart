...
Future<bool> sushiSwitchToAccount(
  ...
  final sessionOk = await sushiRestoreSession(ref, updated);
  if (!sessionOk) {
    if (context.mounted) {
...
Future<void> sushiTapSavedAccount(
  BuildContext context,
  WidgetRef ref,
  AccountModel user,
) async {
  Future<void> switchFn() => sushiSwitchToAccount(context, ref, user);

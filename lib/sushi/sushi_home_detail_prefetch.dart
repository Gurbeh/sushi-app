...
abstract final class SushiHomeDetailPrefetch {
  ...
  static Future<void> _prefetchOne(Ref ref, _PrefetchTarget target) async {
    final cached = await sushiLoadCachedLibraryItemJson(ref, target.itemId);
    if (cached == null) {
      await sushiFetchLibraryItemJson(ref, target.itemId);
    }
        ...
        switch (item) {
          ...
          case SeriesModel s:
            if (!sushiTdlibPlayInProgress()) {
              SushiPlaybackPrefetch.scheduleForSeries(ref.read, s, once: true);
            }
          default:
            break;
        }

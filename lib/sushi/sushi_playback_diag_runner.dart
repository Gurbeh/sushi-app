...
import 'package:fladder/sushi/sushi_playback_diag_hooks.dart';
...
class SushiPlaybackDiagRunner {
  SushiPlaybackDiagRunner(WidgetRef ref) : _ref = ref;
  ...
  Future<String> run({void Function(String phase)? onPhase}) async {
    ...
    if (_cancelled) {
      if (kIsWeb) SushiPlaybackDiagHooks.uninstall();
  ...
  Future<Map<String, Object?>> _appContext() async {
    ...
    return {
      ...
      'isWeb': kIsWeb,
      'sushiEnabled': SushiEnv.isEnabled,
  ...
  Map<String, Object?> _playbackContext() {
    ...
    return {
      ...
      'streamUrl': SushiStreamLog.describeUrl(url),
      'streamHost': SushiStreamLog.describeHost(url),
    };
  }

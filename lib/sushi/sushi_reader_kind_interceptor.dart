...
const sushiReaderKindHeader = 'X-OX-Reader-Kind';

/// Adds [sushiReaderKindHeader] to every PlaybackInfo POST, read fresh (ground truth, not a
...
class SushiReaderKindInterceptor implements Interceptor {
  @override
  FutureOr<Response<BodyType>> intercept<BodyType>(Chain<BodyType> chain) async {
    ...
    headers[sushiReaderKindHeader] = await _readerKind();
  ...
  Future<String> _readerKind() async {
    ...
    final stored = await SushiLoginKindStore.readCurrent();
    if (stored == SushiLoginKind.bot) return 'bot';
    return 'session';
  }
}

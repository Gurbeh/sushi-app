...
abstract final class SushiTdlibSessionCache {
  ...
  static void put(String telegramUrl, String resolvedUrl, {required bool preferHttpBridge}) {
    if (sushiIsSessionBoundPlaybackUrl(resolvedUrl)) return;
    final key = cacheKey(telegramUrl, preferHttpBridge: preferHttpBridge);
  ...
  ///
  /// A cold play carries `sushi-tg://0/0` — the backend has not committed to a sender yet — so
  ...
  static String? _canonicalTelegramUrl(String? url) {
    final parsed = url == null ? null : sushiParseTelegramDeliveryPath(url.trim());
...
class _Entry {
  ...
  bool get isFresh => DateTime.now().difference(storedAt) < SushiTdlibSessionCache.ttl;
}

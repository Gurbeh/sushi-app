...
import 'package:fladder/sushi/providers/sushi_media_issue_context.dart';
import 'package:fladder/sushi/providers/sushi_media_issue_submit.dart';
  ...
  if (AdaptiveLayout.viewSizeOf(context) != ViewSize.phone) {
    await showModalSideSheet(
      ...
      header: Column(
        ...
        spacing: 8,
        children: [
          Text(context.localized.sushiReportIssueTitle),
          Text(target.displayTitle, style: Theme.of(context).textTheme.headlineSmall),
        ],
      ),
      content: SushiMediaIssueSheet(target: target),
    );
  } else {
    await showBottomSheetPill(
      context: context,
      content: (context, scrollController) => SushiMediaIssueSheet(
        target: target,
...
class SushiMediaIssueSheet extends ConsumerStatefulWidget {
  final SushiMediaIssueTarget target;
  final ScrollController? scrollController;

  const SushiMediaIssueSheet({
...
class _OxMediaIssueSheetState extends ConsumerState<SushiMediaIssueSheet> {
  ...
  Future<void> _submit() async {
    ...
    final notifier = ref.read(sushiMediaIssueSubmitProvider.notifier);
    ...
    if (!mounted) return;
    final state = ref.read(sushiMediaIssueSubmitProvider);
    state.whenOrNull(
      data: (_) {
        Navigator.of(context).pop();
        FladderSnack.show(context.localized.sushiReportIssueSuccess, context: context);
      },
      error: (error, _) {
        FladderSnack.show(
          context.localized.sushiReportIssueFailed,
          context: context,
        );
      },
    );
  ...
  Widget build(BuildContext context) {
    final contextAsync = ref.watch(sushiMediaIssueContextProvider(widget.target));
    final submitState = ref.watch(sushiMediaIssueSubmitProvider);
    ...
    return contextAsync.when(
      ...
      error: (error, _) => Padding(
        ...
        child: Text(
          context.localized.sushiReportIssueContextFailed,
          style: TextStyle(color: Theme.of(context).colorScheme.error),
        ),
      ),
      data: (ctx) {
        final categories = SushiMediaIssueCategory.values;
        ...
        return ListView(
          ...
          children: [
            if (AdaptiveLayout.viewSizeOf(context) == ViewSize.phone) ...[
              Text(
                context.localized.sushiReportIssueTitle,
            ...
            Text(context.localized.sushiReportIssueChooseCategory),
            ...
            if (_selected?.requiresCustomMessage == true) ...[
              ...
              OutlinedTextField(
                ...
                label: context.localized.sushiReportIssueOtherHint,
                maxLines: 3,
              ),
            ],
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                label: Text(context.localized.sushiReportIssueSubmit),
              ),
            ),
  ...
  String _categoryLabel(BuildContext context, SushiMediaIssueCategory category) {
    return switch (category) {
      SushiMediaIssueCategory.brokenSubtitles => context.localized.sushiReportIssueBrokenSubtitles,
      SushiMediaIssueCategory.dualAudioDubbing => context.localized.sushiReportIssueDualAudio,
      SushiMediaIssueCategory.badVideoQuality => context.localized.sushiReportIssueBadVideo,
      SushiMediaIssueCategory.other => context.localized.sushiReportIssueOther,
    };
  }
}

...
final pigeonPlayerSettingsSyncProvider = Provider<void>((ref) {
  Future<void> sendSettings() async {
    ...
    if (!kIsWeb && Platform.isAndroid) {
      try {
        await pigeon.PlayerSettingsPigeon().sendPlayerSettings(
        ...
        // Native channel gone (background/detach) — not an actionable crash.
        if (e.code != 'channel-error') rethrow;
      } on MissingPluginException {
        // Engine torn down before pigeon reply.
      }
    }
  }
  ...
  ref.listen(clientSettingsProvider, (_, __) {
    sendSettings();
  });
  ref.listen(videoPlayerSettingsProvider, (_, __) {
    sendSettings();
  });

...
public class Seq {
	...
	/**
	 * Optional single-threaded Go JNI ingress. When set, GoRefQueue never calls
	 * {@link #destroyRef} on its own finalizer thread; the host must schedule
	 * {@link #releaseOnCallerThread} onto the same OS thread that runs every
	 * other gomobile entry (Sushi: ox-gomobile). Without this, GC-driven
	 * destroyRef races sendText and crashes with
	 * {@code fatal error: bulkBarrierPreWrite: unaligned arguments}.
	 */
	public interface GoRefRelease {
		void release(int refnum);
	}

	private static volatile GoRefRelease releaser;

	public static void setReleaser(GoRefRelease r) {
		releaser = r;
	}

	/** Call only from the process's single gomobile JNI thread. */
	public static void releaseOnCallerThread(int refnum) {
		destroyRef(refnum);
	}
				...
				@Override public void run() {
					while (true) {
						try {
							...
							refs.remove(ref);
							GoRefRelease r = releaser;
							if (r != null) {
								r.release(ref.refnum);
							} else {

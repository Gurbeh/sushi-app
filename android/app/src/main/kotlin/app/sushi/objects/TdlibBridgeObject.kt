package app.sushi.objects

import android.util.Log
import SushiTdlibAuthState
import SushiTdlibAuthStateKind
import SushiTdlibBridgeApi
import SushiTdlibBridgeEvents
import SushiTdlibConnectionHealth
import SushiTdlibDeliveryRef
import SushiTdlibPlaybackSource
import SushiTdlibProviderBot
import android.content.Context
import android.content.Intent
import android.os.Handler
import android.os.Looper
import app.sushi.tdlibbridge.auth.OxTelegramAuthController
import app.sushi.tdlibbridge.auth.TdlibAuthState
import app.sushi.tdlibbridge.media.OxTelegramFileFetcher
import app.sushi.tdlibbridge.player.OxTelegramStreamBridge
import app.sushi.tdlibbridge.player.TdlibHttpBridgeServer
import app.sushi.tdlibbridge.session.GomobileCallGate
import app.sushi.tdlibbridge.session.OxTelegramClient
import app.sushi.tdlibbridge.session.OxTelegramSessionStorage
...
/**
 * Singleton bridging Flutter (SushiTdlibBridgeApi, Pigeon-generated) to ox_tdlib_bridge's Kotlin
 ...
 */
object TdlibBridgeObject : SushiTdlibBridgeApi {
    ...
    private var events: SushiTdlibBridgeEvents? = null
    ...
    @Volatile
    private var lastAuthState: SushiTdlibAuthState =
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.UNINITIALIZED)

    @Volatile
    private var lastConnectionHealth: SushiTdlibConnectionHealth = SushiTdlibConnectionHealth.UNINITIALIZED
    ...
    private val connectionSink = ConnectionSink { state ->
        val mapped = when (state) {
            "connecting" -> SushiTdlibConnectionHealth.CONNECTING
            "ready" -> SushiTdlibConnectionHealth.READY
            "degraded" -> SushiTdlibConnectionHealth.DEGRADED
            else -> SushiTdlibConnectionHealth.UNINITIALIZED
        }
    ...
    /** Call once from MainActivity.configureFlutterEngine, alongside SushiTdlibBridgeApi.setUp. */
    fun attach(context: Context, binaryMessenger: BinaryMessenger) {
        appContext = context.applicationContext
        events = SushiTdlibBridgeEvents(binaryMessenger)
    }
    ...
    override fun currentAuthState(): SushiTdlibAuthState = lastAuthState

    override fun isNativeSessionBot(): Boolean = client?.isBotMode() ?: false

    override fun connectionHealth(): SushiTdlibConnectionHealth = lastConnectionHealth
    ...
    fun hasLiveAuth(): Boolean = lastAuthState.kind == SushiTdlibAuthStateKind.READY
    ...
    private fun clearNativeSession() {
        ...
        lastAuthState = SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.UNINITIALIZED)
        lastConnectionHealth = SushiTdlibConnectionHealth.UNINITIALIZED
        runOnMain {
     ...
     */
    override fun warmDelivery(source: SushiTdlibPlaybackSource, callback: (Result<Unit>) -> Unit) {
        scope.launch {
            runCatching {
                awaitPlaybackTeardown()
                val oxClient = client ?: notConfigured()
    ...
    override fun ensureProviderBotsReady(
        bots: List<SushiTdlibProviderBot>,
    ...
    private fun notConfigured(): Nothing =
        throw IllegalStateException("SushiTdlibBridgeApi.configure() must be called before this method")
}

private fun TdlibAuthState.toPigeon(): SushiTdlibAuthState = when (this) {
    is TdlibAuthState.Uninitialized ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.UNINITIALIZED)
    is TdlibAuthState.WaitingForPhoneNumber ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.WAITING_FOR_PHONE_NUMBER)
    is TdlibAuthState.WaitingForCode ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.WAITING_FOR_CODE)
    is TdlibAuthState.WaitingForPassword ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.WAITING_FOR_PASSWORD, passwordHint = hint)
    is TdlibAuthState.WaitingForQrConfirmation ->
        SushiTdlibAuthState(
            kind = SushiTdlibAuthStateKind.WAITING_FOR_QR_CONFIRMATION,
            qrLoginUrl = loginUrl,
            errorMessage = notice.ifEmpty { null },
        )
    is TdlibAuthState.Ready ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.READY)
    is TdlibAuthState.LoggingOut ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.LOGGING_OUT)
    is TdlibAuthState.Closed ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.CLOSED)
    is TdlibAuthState.Failed ->
        SushiTdlibAuthState(kind = SushiTdlibAuthStateKind.FAILED, errorMessage = error.message)
}

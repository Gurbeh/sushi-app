...
void main() {
  group('sushiPlaybackHydrateRetryAfterSeconds', () {
    test('prefers Retry-After header', () {
      ...
      expect(sushiPlaybackHydrateRetryAfterSeconds(response), 3);
    });
    ...
    test('falls back to JSON retry_after', () {
      ...
      expect(sushiPlaybackHydrateRetryAfterSeconds(response), 7);
    });
    ...
    test('defaults when header and body are missing', () {
      ...
      expect(sushiPlaybackHydrateRetryAfterSeconds(response), sushiPlaybackHydrateRetryAfterDefaultSec);
    });
  });

  group('sushiPollPlaybackInfoUntilReady', () {
    test('retries on 202 then returns 200', () async {
      var calls = 0;
      final result = await sushiPollPlaybackInfoUntilReady(() async {
        calls++;
        if (calls < 3) {
